import urllib,urllib2,re,cookielib,string, urlparse,os,sys,xbmc, xbmcgui, xbmcaddon, xbmcplugin,urlresolver
...
def LISTDISC(mname,murl):
        ...
        if dsc: turl='http://dsc.discovery.com'
        mil=re.findall('military.discovery',murl)
        if mil: turl='http://military.discovery.com'
        sci=re.findall('science.discovery',murl)
        if sci: turl='http://science.discovery.com'
        velo=re.findall('velocity.discovery',murl)
        if velo: turl='http://velocity.discovery.com'
        ap=re.findall('animal.discovery',murl)
        if ap: turl='http://animal.discovery.com'
        ...
        for thumb in Thumb: thumbList.append(thumb)
        ...
        if len(match)==0:
                ...
                for url, name, view in match:
                        ...
                        if Full:
                            for ind in Full:
                                if ind == url: name= name + '  [COLOR red]Full Episode[/COLOR]'
...
def LINKDISC(name,url):
        ...
        for plot in Plot: plotList.append(plot)
        ...
        if Full:
            ...
            #WatchHistory
            if selfAddon.getSetting("whistory") == "true": wh.add_item(name+' '+'[COLOR green]Discovery[/COLOR]', sys.argv[0]+sys.argv[2], infolabels='', img=thumbList[0], fanart='', is_folder=False)
            player.KeepAlive()
            return ok
        else:
            ...
            for i in range(len(match)):
                    ...
                    if selfAddon.getSetting("bit-disc") == "0":
                        if (len(match1)>0): final= 'http://discidevflash-f.akamaihd.net/digmed/hdnet/'+idlist1[i]+'/'+idlist2[i]+'/'+idlist3[i]+'-3500k.mp4?seek=5'
                        elif (len(match1)==0) and (len(match2)>0): final= 'http://discidevflash-f.akamaihd.net/digmed/hdnet/'+idlist1[i]+'/'+idlist2[i]+'/'+idlist3[i]+'-1500k.mp4?seek=5'
                        else: final= 'http://discidevflash-f.akamaihd.net/digmed/hdnet/'+idlist1[i]+'/'+idlist2[i]+'/'+idlist3[i]+'-600k.mp4?seek=5'    
                    elif selfAddon.getSetting("bit-disc") == "1":
                        if (len(match2)>0): final= 'http://discidevflash-f.akamaihd.net/digmed/hdnet/'+idlist1[i]+'/'+idlist2[i]+'/'+idlist3[i]+'-1500k.mp4?seek=5'
                        else: final= 'http://discidevflash-f.akamaihd.net/digmed/hdnet/'+idlist1[i]+'/'+idlist2[i]+'/'+idlist3[i]+'-600k.mp4?seek=5'
                    else: final= 'http://discidevflash-f.akamaihd.net/digmed/hdnet/'+idlist1[i]+'/'+idlist2[i]+'/'+idlist3[i]+'-600k.mp4?seek=5'
                    match2=re.compile('1500k').findall(quality)
                    tot = i + 1
                    # play with bookmark
                    if i==0:     player = playbackengine.PlayWithoutQueueSupport(final, 'plugin.video.movie25', video_type='', title=ETitleList[i],season='', episode='', year='', watch_percent=0.85, watchedCallbackwithParams=main.WatchedCallback, img=thumbList[i], infolabels={'Title':'[COLOR blue]'+ETitleList[i]+'[/COLOR]','Plot': plotList[i],'Genre': '[B]Clip '+str(tot)+'/'+str(len(match))+' on playlist[/B]        '+mtitle})
                    else: playbackengine.AddToPL(ETitleList[i], final, img=thumbList[i], infolabels={'Title':'[COLOR blue]'+ETitleList[i]+'[/COLOR]','Plot': plotList[i],'Genre': '[B]Clip '+str(tot)+'/'+str(len(match))+' on playlist[/B]        '+mtitle})
                    i=i+1
            xbmc.Player().pause()
            if idlist3[0][0:7]==idlist3[1][0:7]: xbmc.executebuiltin("XBMC.Notification([B]Attention![/B],"+str(len(match))+" Clips loaded to playlist,10000)")
            elif idlist3[0][6:13]==idlist3[1][6:13]: xbmc.executebuiltin("XBMC.Notification([B]Attention![/B],"+str(len(match))+" Clips loaded to playlist,10000)")
            elif idlist3[1][6:13]==idlist3[2][6:13]: xbmc.executebuiltin("XBMC.Notification([B]Attention![/B],"+str(len(match))+" Clips loaded to playlist,10000)")
            else: xbmc.executebuiltin("XBMC.Notification([B]Attention![/B],Related clips loaded to playlist,10000)")
            #WatchHistory
            if selfAddon.getSetting("whistory") == "true": wh.add_item(name+' '+'[COLOR green]Discovery[/COLOR]', sys.argv[0]+sys.argv[2], infolabels='', img=thumbList[0], fanart='', is_folder=False)
            player.KeepAlive()
            return ok