...
def getResponse(url, size, referrer):
    try:
        ...
        if len(referrer) > 0: req.add_header('Referer', referrer)
        if size > 0:
            size = int(size)
...
def download(url, dest, title=None, referrer=None):
    if not title: title  = 'XBMC Download'
...
def doDownload(url, dest, title, referrer):
    ...
    resume  = 0
    sleep   = 0
    if xbmcgui.Dialog().yesno(title + ' - Confirm Download', file, 'Complete file is %dMB' % mb, 'Continue with download?', 'Confirm',  'Cancel') == 1: return
    f = open(dest, mode='wb')
    chunk  = None
    chunks = []
    while True:
        ...
        try:        
            ...
            if not chunk:                
                if percent < 99: error = True
                    ...
                    if not xbmc.Player().isPlaying(): xbmcgui.Dialog().ok(title, dest, '' , 'Download finished')
                    return
        except Exception, e:
            print str(e)
            error = True
            sleep = 10
            errno = 0
            if hasattr(e, 'errno'): errno = e.errno
            if errno == 10035: pass
