...
def SearchhistoryPFTV(index=False):
        ...
        if not os.path.exists(SeaFile): SEARCHWS(index=index)
        else:
...
def superSearch(encode,type):
    try:
        ...
        if epi:
            ...
            for url,nameds in match: url = BASE_URL2 + url+'/season_'+ss+'.html'
...
def AtoZPFTV(index=False):
    ...
    for i in string.ascii_uppercase: main.addDir(i,(chr(ord(i)+1)),464,art+'/'+i.lower()+'.png',index=index)
    main.VIEWSB()

def LISTSHOW(mname,murl,index=False):
    link=main.OPENURL(BASE_URL)
    link=link.replace('\r','').replace('\n','').replace('\t','')
    if 'Z' in mname: links = re.search('<a name="Z">(.+?)<!-- End of the page footer -->', link)
    elif '0-9' in mname: links = re.search('<a name="#">(.+?)<a name="A">', link)
    else: links = re.search('<a name="'+mname+'">(.+?)<a name="'+murl+'">', link)
    if links:
        links = links.group(1)
        match=re.compile('''class="mnlcategorylist"><a href="([^"]+?)"><b>([^<]+?)</b></a>''', re.DOTALL).findall(links)
        for url, name in match: main.addDirT(name,BASE_URL+url,465,'','','','','','',index=index)

def POPULARPFTV(murl,index=False):
        link=main.OPENURL(BASE_URL2)
        link=link.replace('\r','').replace('\n','').replace('\t','')
        if 'season' in murl: match=re.compile('(?sim)<td class="tleft" style="text-align:center"><a href="([^"]+?)">([^<]+?)</a>').findall(link)
        else: match=re.compile('(?sim)<td class="tleft" style="text-align:center;"><a href="([^"]+?)">([^<]+?)</a>').findall(link)
        for url,name in match:
            if 'season' in murl:
                    ...
                    if index == 'True': main.addDirTE(fname,BASE_URL2+url,21,'','','','','','')
                    else: main.addDirTE(fname,BASE_URL2+url,461,'','','','','','')
            else: main.addDirT(name,url,465,'','','','','','',index=index)
...
def LISTSEASON(mname,murl,index=False):
        ...
        for url, name, count in reversed(match): main.addDir(mname+' [COLOR red]'+name+'[/COLOR] '+count,murl+url,466,thumb,index=index)
