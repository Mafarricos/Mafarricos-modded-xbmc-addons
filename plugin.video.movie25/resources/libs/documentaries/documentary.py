...
def LISTDOC(murl):
    if murl=='doc1':
        ...
        for url, name in match: main.addDir(name,url,87,'')
    elif murl=='doc3':
        ...
        for url, desc, name, leng in match: main.addDirc(name+'  '+leng,url,87,'',desc,'','','','')
...
def LISTDOC2(murl):
    ...
    if (len(match)>0):
        ...
        if (len(match)==0): match=re.compile('href="(.+?)" title="" rel=".+?"><img class=".+?" src="(.+?)" alt="(.+?)".+?</a>\n                            </div>     \n                            <div id="postDis">\n                            \t(.+?)[...]').findall(link)
        for url,thumb,name,desc in match: main.addPlayMs(name,url,88,thumb,desc,'','','','')
        paginate=re.compile('<a href="([^<]+)" >Next &rarr;</a>').findall(link)
        if (len(paginate)>0): main.addDir('[COLOR blue]Next Page[/COLOR]',paginate[0],87,"%s/art/next2.png"%selfAddon.getAddonInfo("path"))
    ...
    if (len(match2)>0):
        ...
        if (len(paginate)>0):
            for purl in paginate: main.addDir('[COLOR blue]Next[/COLOR]',purl,87,"%s/art/next2.png"%selfAddon.getAddonInfo("path"))
...
def LISTDOCPOP(murl):
    if murl=='search':
        ...
        if (len(paginate)>0): main.addDir('[COLOR blue]Page 2[/COLOR]','http://documentaryheaven.com/page/2/?s='+encode,9,"%s/art/next2.png"%selfAddon.getAddonInfo("path"))
    elif murl=='rec':
        rurl='http://topdocumentaryfilms.com/'
        link=main.OPENURL(rurl)
        match=re.compile('href="([^<]+)">([^<]+)</a></li><li><a').findall(link)
        for url,name in match: main.addPlayMs(name,url,88,'','','','','','')
    else:
        link=main.OPENURL(murl)
        link=link.replace('\r','').replace('\n','').replace('\t','').replace('&nbsp;','')
...
def LINKDOC(mname,murl,thumb):
    ...
    if (len(match)>0):
        ...
        if (len(match)==0):
            ...
            if (len(match)==0): match=re.compile('<embe.+?src="http:([^<]+)".+?></embed>').findall(link)
        for url in match:
            url='http:'+url
            ...
            if (len(match5)>0): url=url.replace('http://www.dailymotion.com/embed/video','http://www.dailymotion.com/video')
        ...
        if (len(match)==0):
            ...
            for url in match:
                ...
                if (len(match4)>0):
                    url=url.replace('?title=0&amp;byline=0&amp;portrait=0','')
                    url=url.replace('http://player.vimeo.com/video','http://vimeo.com')
                elif (len(match6)>0): url=url
                else:
                    match2=re.compile('http://www.youtube.com/embed/([^<]+)').findall(url)
                    if (len(match2)==0): match2=re.compile('http://www.youtube.com/p/([^<]+).?hl=.+?').findall(link)
                    url='http://www.youtube.com/watch?v='+match2[0]
        ...
        try:
            ...
            #WatchHistory
            if selfAddon.getSetting("whistory") == "true": wh.add_item(mname+' '+'[COLOR green]Doc-Heaven[/COLOR]', sys.argv[0]+sys.argv[2], infolabels='', img=thumb, fanart='', is_folder=False)
            player.KeepAlive()
            return ok
        except:
    ...
    if (len(match2)>0):
            ...
            if (len(match6)>0):
                ...
                if (len(match)>0): url='http://www.youtube.com/watch?feature=player_embedded&v=n_'+match[0]
                    ...
                    if (len(match2)>0):
                        ...
                        print match3[0]
                        try: url='http://www.youtube.com/watch?v='+match2[0]
                        except:
                                namelist=[]
                                urllist=[]
        ...
        try:
            ...
            player = playbackengine.PlayWithoutQueueSupport(resolved_url=stream_url, addon_id=addon_id, video_type='', title=mname,season='', episode='', year='',img=thumb,infolabels='', watchedCallbackwithParams=main.WatchedCallbackwithParams,imdb_id='')
            #WatchHistory
            if selfAddon.getSetting("whistory") == "true": wh.add_item(mname+' '+'[COLOR green]Top-Doc[/COLOR]', sys.argv[0]+sys.argv[2], infolabels='', img=thumb, fanart='', is_folder=False)
            player.KeepAlive()
            return ok
        except:
            xbmc.executebuiltin("XBMC.Notification(Sorry!,Link Not Playable,5000)")
    ...
    if (len(match3)>0):        
        ...
        try:
            ...
            player = playbackengine.PlayWithoutQueueSupport(resolved_url=stream_url, addon_id=addon_id, video_type='', title=mname,season='', episode='', year='',img=thumb,infolabels='', watchedCallbackwithParams=main.WatchedCallbackwithParams,imdb_id='')
            #WatchHistory
            if selfAddon.getSetting("whistory") == "true": wh.add_item(mname+' '+'[COLOR green]Doc-Log[/COLOR]', sys.argv[0]+sys.argv[2], infolabels='', img=thumb, fanart='', is_folder=False)
            player.KeepAlive()
            return ok
        except Exception, e:
            if stream_url != False: main.ErrorReport(e)
            else: xbmc.executebuiltin("XBMC.Notification(Sorry!,Link Not Playable,5000)")
            return ok