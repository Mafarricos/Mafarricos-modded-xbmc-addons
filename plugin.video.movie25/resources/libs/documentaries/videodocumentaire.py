...
def MAINVD():
        ...
        for url,name,thumb in match: main.addDir(name,url,332,thumb)
...
def SEARCHVD():
        ...
        if (keyb.isConfirmed()):
            ...
            for url,thumb,name,desc in match: main.addPlayMs(name,url,333,thumb,desc,'','','','')
...
def LISTVD(murl):
        ...
        for thumb,dur,url,name,desc in match: main.addPlayMs(name+' [COLOR red]('+dur+')[/COLOR]',url,333,thumb,desc,'','','','')
        paginate = re.compile('href="([^<]+)" >Next</a>',re.DOTALL).findall(link)
        if len(paginate)>0: main.addDir('Next',paginate[0],332,art+'/next2.png')
...
def LINKVD(name,murl,thumb):
        ...
        try:
            ...
            if 'http:' not in match: match='http:'+match
        ...
        try:
                    ...
                    #WatchHistory
                    if selfAddon.getSetting("whistory") == "true": wh.add_item(name+' '+'[COLOR green]Video Documentaire[/COLOR]', sys.argv[0]+sys.argv[2], infolabels=infoL, img=thumb, fanart='', is_folder=False)
                    player.KeepAlive()
                    return ok
        except Exception, e:
