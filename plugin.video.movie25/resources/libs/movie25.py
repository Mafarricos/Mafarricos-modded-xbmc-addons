...
def UFCMOVIE25():
    surl=MainUrl+'/search.php?key=ufc&submit='
    ...
    dialogWait.close()
    del dialogWait
    main.addDir('[COLOR blue]Page 2[/COLOR]',MainUrl+'/search.php?page=2&key=ufc',9,art+'/next2.png')

def Searchhistory(index=False):
    seapath=os.path.join(main.datapath,'Search')
    SeaFile=os.path.join(seapath,'SearchHistory25')
    if not os.path.exists(SeaFile): SEARCH(index=index)
    else:
...
def SEARCH(murl = '',index=False):
    ...
    if exist:
        ...
        if r: main.addDir('[COLOR blue]Page 1 of '+r[0]+'[/COLOR]',MainUrl+'/search.php?page=2&key='+encode,9,art+'/next2.png',index=index)
        else: main.addDir('[COLOR blue]Page 1[/COLOR]',MainUrl+'/search.php?page=2&key='+encode,9,art+'/next2.png',index=index)
    xbmcplugin.setContent(int(sys.argv[1]), 'Movies')
...
def YEARB(murl,index=False):
    ...
    if r:
        ...
        main.addDir('[COLOR blue]Page 1 of '+r[0]+'[/COLOR]',MainUrl+'/search.php?page=2&year='+str(ye),9,art+'/next2.png',index=index)    
    else: main.addDir('[COLOR blue]Page 1[/COLOR]',MainUrl+'/search.php?page=2&year='+str(ye),9,art+'/next2.png',index=index)
    xbmcplugin.setContent(int(sys.argv[1]), 'Movies')
    main.VIEWS()
...
def NEXTPAGE(murl,index=False):
    ...
    matchx=re.compile(MainUrl+'/search.php.+?page=(.+?)&year=(.+?)').findall(murl)
    if len(matchx)>0:
        durl = murl + '/'
        paginate=re.compile(MainUrl+'/search.php.+?page=(.+?)&year=(.+?)/').findall(durl)
        for page, yearb in paginate:
            pgs = int(page)+1
            jurl=MainUrl+'/search.php?page='+str(pgs)+'&year='+str(yearb)
#                 main.addDir('[COLOR red]Home[/COLOR]','',0,art+'/home.png')
        ...
        else: main.addDir('[COLOR blue]Page '+str(page)+'[/COLOR]',jurl,9,art+'/next2.png',index=index)
        xbmcplugin.setContent(int(sys.argv[1]), 'Movies')
        main.VIEWS()                
    else:
        durl = murl + '/'
        paginate=re.compile(MainUrl+'/search.php.+?page=(.+?)&key=(.+?)/').findall(durl)
        for page, search in paginate:
            pgs = int(page)+1
            jurl=MainUrl+'/search.php?page='+str(pgs)+'&key='+str(search)
#                 main.addDir('[COLOR red]Home[/COLOR]','',0,art+'/home.png')
        r = re.findall(""">Next</a><a href='search.php.?page=([^<]+)&key=.+?'>Last</a>""",link)
        if r: main.addDir('[COLOR blue]Page '+str(page)+' of '+r[0]+'[/COLOR]',jurl,9,art+'/next2.png',index=index)
        else: main.addDir('[COLOR blue]Page '+str(page)+'[/COLOR]',jurl,9,art+'/next2.png',index=index)
...
def GroupedHosts(name,url,thumb):
    ...
    urls = eval(url)
    for url in urls: main.addDown2(name,MainUrl+url,5,thumb,thumb)
