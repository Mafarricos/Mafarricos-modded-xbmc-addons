...
def DISJRList(murl):
        if murl=='music':
            ...
            for url,name,thumb in match:
                if 'http' in url: url=url
                else: url='http://disneyjunior.com/_grill/json'+url+'/video?r=1-1&l=31&o=0'
...
def DISJRList2(murl):
            ...
            for url, name,thumb,desc,vtype in match: main.addPlayMs('[COLOR red]'+name+'[/COLOR] [COLOR yellow]"'+vtype+'"[/COLOR]',url,110,thumb,desc,'','','','')
...
def DISJRLink(mname,murl,thumb):
        ...
        playlist.clear()
        if 'http' not in murl: murl='http://disneyjunior.com'+murl
        link=main.OPENURL(murl)
        stream_url = 'rtmp://videodolimgfs.fplive.net/videodolimg'
        video = re.compile('{"bitrate":.+?,"format":"mp4","url":"mp4:(.+?)","id".+?}').findall(link)
        if video:
                if selfAddon.getSetting("disj-qua") == "0": vid=video[len(video)-1]
                elif selfAddon.getSetting("disj-qua") == "1": vid=video[len(video)-5]
                elif selfAddon.getSetting("disj-qua") == "2": vid=video[0]
        ...
        #WatchHistory
        if selfAddon.getSetting("whistory") == "true": wh.add_item(mname+' '+'[COLOR green]Disney jr[/COLOR]', sys.argv[0]+sys.argv[2], infolabels='', img=thumb, fanart='', is_folder=False)
        player.KeepAlive()
        return ok
