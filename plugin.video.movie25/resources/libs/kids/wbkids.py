...
def LISTWB(murl):
    ...
    for name,desc,url,thumb in match: main.addPlayMs(name,url,79,thumb,desc,'','','','')
...
def LINKWB(mname,murl):
        ...
        if (len(bit700)>0): stream_url=bit700[0]
        else: stream_url=bit500[0]
        ...
        #WatchHistory
        if selfAddon.getSetting("whistory") == "true": wh.add_item(mname+' '+'[COLOR green]WB Kids[/COLOR]', sys.argv[0]+sys.argv[2], infolabels='', img=thumb[0], fanart='', is_folder=False)
        player.KeepAlive()
        return ok
